...
class Setup
{
    public const CMS_DEFAULT_LANGUAGE_ID = 24;
    ...
    {
        $composerJsonPath = 'COMPOSER='.__DIR__.'/composer.json';
        $getPhpBinaryPath = shell_exec('which php');
        $composer = __DIR__.'/setup/bin/composer.phar';
        $composerInstallCommand = sprintf('%s %s %s install', $composerJsonPath, $getPhpBinaryPath, $composer);
        var_dump($composerInstallCommand);

        $output = '';
       // exec($composerInstallCommand, $output);
